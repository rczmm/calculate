...
function submit_question(ev: Event): void {
  ...
  const answerValue = (formData.get("answer") as string).trim();
  const isCorrect = question.answer_current_question(answerValue);
  if (isCorrect) {
    status.value = Status.Ready;
...
<template>
  <div class="exercise pt-12">
    <el-button type="info" class="absolute left-6 top-4" @click="go_back">返回</el-button>
    <div class="mx-24 text-2xl">
      <el-progress
          :percentage="question.passedRatio * 100"
          :color="customColor"
          :show-text="false"
          :text-inside="true"
          :stroke-width="20"
      >
        <span>正确率：{{ question.passedRatio * 100 }} %</span>
      </el-progress>
      <div class="text-center mt-2">
        {{ question.passedCnt }} / {{ setting.quantity }}
      </div>
    </div>
    <el-card class="w-max mx-auto my-4">
    ...
    <el-form class="text-2xl" @submit.prevent="submit_question">
      ...
      <div class="px-2 py-2 flex flex-nowrap gap-1 lg:ml-24 lg:mr-12 sm:ml-12 sm:mr-2">
        <el-input
            ...
            ref="answerInput"
        ></el-input>
        <el-button type="primary" class="w-max" native-type="submit">提交</el-button>
      </div>
    </el-form>
    <div v-if="!started">
      <el-button type="success" @click="start">开始</el-button>
      <el-button type="info" class="ml-2" @click="go_to_print_page">打印</el-button>
    </div>
  </div>
</template>
